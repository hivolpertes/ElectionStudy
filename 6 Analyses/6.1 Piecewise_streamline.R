...
rest = lmer(PA_agg ~ 1 + B1 + B2 + (1|SubID), data = dat)
#effect size
(r.squaredGLMM(full)[1] - r.squaredGLMM(rest)[1])/(1-r.squaredGLMM(full)[1]) # fixed
(r.squaredGLMM(full)[2] - r.squaredGLMM(rest)[2])/(1-r.squaredGLMM(full)[2]) # fixed and random
#CI
confint(full)
...
ggsave("./6 Daily diary/3 Piecewise plots/PiecewisePlot_fullsample.jpg", plot, units="in", width=8, height=5.8)
...
rest = lmer(NA_agg ~ 1 + B1 + B2*Trump + B3*Trump + (1|SubID), data = dat) %>% r.squaredGLMM()
